...
rule gather_reports:
    ...
    run:
        with open("temp_report.txt","w") as fw:
        ...
        shell("cat temp_report.txt {input} > {output} && rm temp_report.txt")
...
rule gather_counts:
    ...
    run:
        with open("temp_count.txt","w") as fw:
            fw.write("sample,cpg_site,total,c,t,a,g,-\n")

        shell("cat temp_count.txt {input} > {output} && rm temp_count.txt")